      ...
      <div className="max-w-7xl mx-auto px-4 pt-24 pb-32">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-32">
          {[
            ...
            <div key={i} className="p-8 border border-gray-100 dark:border-slate-700 rounded-3xl hover:border-indigo-200 transition-all space-y-4 shadow-sm shadow-gray-50">
        ...
        <div className="space-y-8">
          <div className="flex justify-between items-end">
            ...
            <p className="text-gray-500 dark:text-gray-400 font-medium">{t('careers.locationInfo')}</p>
          </div>
          ...
          <div className="grid grid-cols-1 gap-4">
            {jobs.map((job, i) => (
              <motion.div 
                key={i}
                whileHover={{ x: 10 }}
                className="p-8 bg-gray-50 dark:bg-slate-800 rounded-3xl flex flex-col md:flex-row md:items-center justify-between gap-6 cursor-pointer group hover:bg-indigo-50 transition-colors"
              >
                ...
                <button className="bg-white dark:bg-slate-900 px-6 py-3 rounded-xl border border-gray-200 dark:border-slate-600 font-bold group-hover:border-indigo-600 group-hover:text-indigo-600 transition-all">
                  {t('careers.apply')}
                </button>
              </motion.div>
            ))}
