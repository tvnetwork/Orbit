...
export default function Community() {
  ...
  return (
    <div className="bg-gray-50 dark:bg-slate-800 min-h-screen pt-12">
      <div className="max-w-7xl mx-auto px-4 lg:grid lg:grid-cols-12 gap-10">
        ...
        <div className="hidden lg:block lg:col-span-3 space-y-8 sticky top-28 h-fit">
          <div className="bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] border border-gray-100 dark:border-slate-700 shadow-sm">
             <h3 className="text-xl font-black text-gray-900 dark:text-white tracking-tight mb-6">Explore</h3>
             <div className="space-y-2">
                <button 
                  onClick={() => navigate('/community')}
                  className={cn(
                    "w-full flex items-center gap-3 px-4 py-3 rounded-xl font-bold text-sm transition-all",
                    !selectedCohortId ? "bg-indigo-50 text-indigo-600" : "text-gray-400 dark:text-gray-500 hover:bg-gray-50 dark:bg-slate-800"
                  )}
                >
                ...
                <button 
                  ...
                  className="w-full flex items-center gap-3 px-4 py-3 rounded-xl font-bold text-sm text-gray-400 dark:text-gray-500 hover:bg-gray-50 dark:bg-slate-800 transition-all"
                >
                ...
                {mentors.map(mentor => (
                  <Link key={mentor.id} to={`/freelancer/${mentor.id}`} className="flex items-center gap-3 group">
                    <img 
                      ...
                      className="h-10 w-10 bg-white dark:bg-slate-900/20 rounded-xl object-cover grayscale group-hover:grayscale-0 transition-all shadow-sm" 
                      alt=""
                    />
                    <div>
        ...
        <div className="col-span-12 lg:col-span-6 space-y-6 h-[calc(100vh-140px)] flex flex-col">
          <div className="bg-white dark:bg-slate-900 p-6 rounded-[2.5rem] border border-gray-100 dark:border-slate-700 shadow-sm flex items-center justify-between shrink-0">
             ...
             <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-black text-gray-500 dark:text-gray-400 uppercase tracking-widest">{onlineCount} Domain Masters Online</span>
             </div>
          </div>

          {/* Chat Bubble Area */}
          <div className="flex-1 bg-white dark:bg-slate-900 rounded-[3rem] border border-gray-100 dark:border-slate-700 shadow-sm overflow-hidden flex flex-col relative">
                  ...
                  return (
                    ...
                    >
                      <img 
                        ...
                        className="h-10 w-10 rounded-xl shadow-sm border border-gray-100 dark:border-slate-700 shrink-0 object-cover" 
                        alt=""
                      ...
                      <div className={cn(
                        ...
                        <p className="text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-widest mb-1 mx-1">{msg.senderName}</p>
                        <div className={cn(
                          "max-w-md px-6 py-4 rounded-[2rem] shadow-sm text-sm font-medium leading-relaxed",
                          isMine ? "bg-indigo-600 text-white rounded-tr-none" : "bg-gray-50 dark:bg-slate-800 text-gray-900 dark:text-white border border-gray-100 dark:border-slate-700 rounded-tl-none"
                        )}>
                          {msg.text}
                        </div>
             ...
             {/* Input */}
             <div className="p-8 border-t border-gray-100 dark:border-slate-700 bg-white dark:bg-slate-900">
                <form onSubmit={handleSendMessage} className="flex gap-4">
                   <div className="flex-1 relative">
                      <input 
                        ...
                        className="w-full bg-gray-50 dark:bg-slate-800 border border-gray-100 dark:border-slate-700 rounded-[2rem] px-8 py-5 text-sm font-medium focus:outline-none focus:ring-4 focus:ring-indigo-50 focus:bg-white dark:bg-slate-900 focus:border-indigo-300 transition-all shadow-inner"
                      />
                   </div>
                   <button 
                    type="submit"
        ...
        <div className="hidden lg:block lg:col-span-3 space-y-8 sticky top-28 h-fit">
          <div className="bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] border border-gray-100 dark:border-slate-700 shadow-sm">
             <h3 className="text-xl font-black text-gray-900 dark:text-white tracking-tight mb-8">Active Fields</h3>
                ...
                {[
                  ...
                  <div key={trend.tag} className="flex items-center justify-between group cursor-pointer">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-sm font-black text-gray-900 dark:text-white group-hover:text-indigo-600 transition-colors">#{trend.tag}</span>
                    </div>
          ...
          <div className="space-y-6">
             <h3 className="text-lg font-black text-gray-900 dark:text-white tracking-tight px-2 flex items-center justify-between">
             ...
             <div className="space-y-4">
                {cohorts.map(cohort => (
                  <Link 
                    ...
                    className="flex items-center gap-4 p-4 bg-white dark:bg-slate-900 border border-gray-100 dark:border-slate-700 rounded-3xl hover:shadow-xl hover:shadow-indigo-500/5 transition-all group"
                  >
                    ...
                    <div className="min-w-0">
                      <h4 className="font-black text-gray-900 dark:text-white text-sm truncate">{cohort.name}</h4>
                      <p className="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest">{cohort.membersCount} Members</p>
                    </div>
                  </Link>
                ))}
             </div>
